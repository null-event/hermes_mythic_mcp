...
#commented out run_as_user and execute_mimikatz - not relevant for macOS red teaming
"""
@mcp.tool()
...
async def get_all_payloads():
    """Returns a list of all payloads"""

    output = ""

    payloads = await api.get_all_payloads()

    for payload in payloads:
        output += f"ID: {payload['id']}\n"
        output += f"Name: {payload['name']}\n"
        output += f"Type: {payload['type']}\n"
...
@mcp.tool()
async def get_env(agent_id: int, command: str):
    """Returns the environment variables for the current session"""

    output = await api.get_env(agent_id, command)

    return f"---\n{output}\n---"

@mcp.tool()
async def fda_check(agent_id: int, command: str):
    """Checks if the current user has full disk access"""

    output = await api.fda_check(agent_id, command)

    return f"---\n{output}\n---"

@mcp.tool()
async def accessibility_check(agent_id: int, command: str):
    """Checks if the current user has accessibility permissions - uses AXIsProcessTrusted()"""

    output = await api.accessibility_check(agent_id, command)

    return f"---\n{output}\n---"

@mcp.tool()
async def get_execution_context(agent_id: int, command: str):
    """Returns the execution context for the current session"""

    output = await api.get_execution_context(agent_id, command)

    return f"---\n{output}\n---"

@mcp.tool()
async def ifconfig(agent_id: int, command: str):
    """Gather IP addresses information for the current session"""

    output = await api.ifconfig(agent_id, command)

    return f"---\n{output}\n---"

@mcp.tool()
async def list_apps(agent_id: int, command: str):
    """List running applications with NSApplication.RunningApplications"""

    output = await api.list_apps(agent_id, command)

    return f"---\n{output}\n---"

@mcp.tool()
async def list_processes(agent_id: int, command: str):
    """Lists all running processes information"""

    output = await api.ps(agent_id, command)

    return f"---\n{output}\n---"

@mcp.tool()
async def take_screenshot(agent_id: int, command: str):
    """Captures a screenshot of the current session - requires Screen Recording permissions"""

    output = await api.screenshot(agent_id, command)

    return f"---\n{output}\n---"

@mcp.tool()
async def tcc_folder_checker(agent_id: int, command: str):
    """Uses mdquery API to check access to TCC-protected macOS folders: ~/Downloads, ~/Desktop, ~/Documents"""

    output = await api.tcc_folder_checker(agent_id, command)

    return f"---\n{output}\n---"

@mcp.tool()
async def whoami(agent_id: int, command: str):
    """Gathers current user context"""

    output = await api.whoami(agent_id, command)
